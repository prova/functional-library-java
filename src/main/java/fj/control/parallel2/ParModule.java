...
import static fj.control.parallel2.Promise.liftM2;
...
public final class ParModule {
  ...
  public <A, B, C> F2<A, B, Promise<C>> promise(final F2<A, B, C> f) {
    return P2.untuple( Promise.promise(strategy, f.tuple()));
  }
  ...
  public <A, B> Promise<List<B>> mapM(final List<A> as, final F<A, Promise<B>> f) {
      final List<Promise<B>> map = as.map(f);
      final F<List<Promise<B>>, Promise<List<B>>> promise = promise(new F<List<Promise<B>>, List<B>>() {
          @Override
          public List<B> f(final List<Promise<B>> promises) {
              return promises.map(new F<Promise<B>, B>() {
                  @Override
                  public B f(final Promise<B> bPromise) {
                      return bPromise.claim();
                  }
              });
          }
      });
      return promise.f(map);
  }
  /**
